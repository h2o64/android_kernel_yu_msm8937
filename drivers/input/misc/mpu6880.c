...
#define MPU6880_ACCEL_SCALE_SHIFT_2G	0  ///4
				   ...
				   const struct i2c_device_id *id)
{
	...
	ret = sensors_classdev_register(&sensor->gyro_dev->dev,& sensor->gyro_cdev);
	if (ret) {
		dev_err(&client->dev,
	...
	ret = sensors_classdev_register(&sensor->accel_dev->dev,&sensor->accel_cdev);
	if (ret) {
		dev_err(&client->dev,
			"create accel class device file failed!\n");
		ret = -EINVAL;
